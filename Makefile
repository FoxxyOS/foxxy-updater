...
install:
	...
	cp dist-upgrade.sh $(DESTDIR)/usr/bin/foxxy-upgrade
	cp foxxy-updater.desktop $(DESTDIR)/etc/xdg/autostart/
	chown root:root $(DESTDIR)/usr/bin/update-reminder
	chown root:root $(DESTDIR)/usr/bin/dist-upgrade
	chown root:root $(DESTDIR)/etc/xdg/autostart/foxxy-updater.desktop
	...
	chmod 755 $(DESTDIR)/usr/bin/foxxy-upgrade
	chmod 755 $(DESTDIR)/etc/xdg/autostart/foxxy-updater.desktop
